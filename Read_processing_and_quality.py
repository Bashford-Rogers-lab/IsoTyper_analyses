...
def QC_samples(dir,gene,id,source,length,species,barcode_group,quasr_location):
  pre = source
  ...
  if(1==1):
    ...
    if(1==1):
      ...
      command1 = "java -jar "+quasr_location+"qualityControl.jar -f "+reads1 +" -o "+dir+"FASTQ_FILES/Sequences_"+id+"_1 -m "+threshold+" -l "+length
      command2 = "java -jar "+quasr_location+"qualityControl.jar -f "+reads2 +" -o "+dir+"FASTQ_FILES/Sequences_"+id+"_2 -m "+threshold+" -l "+length
...
def Trim_sequences_BCR_TCR(Tmp_file,Fail_file,Output_trim, gene,paired,species,primer_file,primer_tag_file,tmp_file,primer_tag_file_count,sample,ref_const,reverse_primer_group,cd_hit_directory):
  (rc)= Init_rc()
...
def Blast_match_J(out, seqs, Trim1, Trim2, refj,e_value, blastall,refv,primer_length):
  ...
  commands.getoutput(command1)
  command2 =  blastall+ " -db "+refv+" -evalue "+str(e_value)+" -query "+Trim1+"_blast_J -out  "+Trim1+"_blast_V_results -max_target_seqs 1 -outfmt 6 -word_size 4"
...
def Get_codons ():
  fh = open("/users/immune-rep/mfj169/REPERTOIRE_ANALYSES/LIBRARY/Codon_table2.txt","r")
  codon = {}
  for l in fh:
    l=l.strip()
    l1 = l.split()
...
def Get_read_report(Seq_file1,Seq_file2,Tmp_file,Trim1, nn_orf_filtered,filtering_report,id, species, gene, dir,primer_tag_file_count,primer_file,method,barcode_group,Trim3):
  ...
  if(barcoded_j+barcoded_v==0):
    raw1,raw2,joined,gene_matching,nORF=Get_number_sequences(Seq_file1), Get_number_sequences(Seq_file2), Get_number_sequences(Tmp_file), Get_reduced_number_sequences_multi_constants(Trim1), Get_reduced_number_sequences_multi_constants(Trim3)
    orf = Get_reduced_number_sequences_multi_constants(nn_orf_filtered)
    number_unique_seqs = Get_number_sequences(nn_orf_filtered)
    out = "Directory\tSample\tSpecies\tGene\t% reads retained\tN raw reads (1)\tN raw reads (2)\tN joined reads\tN reads gene matched\tN reads pre-ORF\tN reads filtered\tUnique sequences\n"
    out=out+dir+"\t"+id+"\t"+species+"\t"+gene+"\t"+str(orf*100.0/min([raw1,raw2]))+"\t"+str(raw1)+"\t"+str(raw2)+"\t"+str(joined)+"\t"+str(gene_matching)+"\t"+str(nORF)+"\t"+str(orf)+"\t"+str(number_unique_seqs)+"\n"
  else:
    Tmp_file1 = Tmp_file.replace(".fasta","_CHECKED.fasta")
    print Tmp_file1
    ...
    raw1,raw2,joined,gene_matching=Get_number_sequences(Seq_file1), Get_number_sequences(Seq_file2), Get_number_sequences_unique(Tmp_file1), Get_reduced_number_sequences_multi_constants(Trim1)
    #count_bc_found,count_uniq_bcs = Count_raw_barcoded_sequences(primer_tag_file_count) #### allow for barcoded sequences
    count_bc_found,count_uniq_bcs = -1,-1
    nORF=Get_reduced_number_sequences_multi_constants(Trim3)
    orf = Get_reduced_number_sequences_multi_constants(nn_orf_filtered)
    number_unique_seqs = Get_number_sequences(nn_orf_filtered)
    out = "Directory\tSample\tSpecies\tGene\t% reads retained\tN raw reads (1)\tN raw reads (2)\tN joined reads\tN reads with BCs\tN uniq BCs\tN reads gene matched\tN reads pre-ORF\tN reads filtered\tUnique sequences\n"
    orf_perc = str(orf*100.0/min([raw1,raw2]))
    if(min([raw1,raw2])==-1):orf_perc = "NA"
    out=out+dir+"\t"+id+"\t"+species+"\t"+gene+"\t"+orf_perc+"\t"+str(raw1)+"\t"+str(raw2)+"\t"+str(joined)+"\t"+str(n_barcodes)+"\t"+str(uniq_sequences)+"\t"+str(gene_matching)+"\t"+str(nORF)+"\t"+str(orf)+"\t"+str(number_unique_seqs)+"\n"
...
quasr_location = locations["quasr_location"] #"/users/immune-rep/mfj169/INSTALLED_PROGRAMS/QUASR_v7.01/"
ref_dir = locations["ref_library"]#"/users/immune-rep/mfj169/REPERTOIRE_ANALYSES/LIBRARY/"
...
if(command_source.count("1")!=0):
  Intialise_files(dir)
  QC_samples(dir,gene,id,source,length,species,barcode_group,quasr_location)
...
if(command_source.count("2")!=0):
  if(gene.count("IG")!=0):
    Split_reads_and_join(Seq_file1, Seq_file2, Tmp_file,gene,paired,id,method,primer_file,barcode_group,sys.argv[12],dir) 
    Trim_sequences_BCR_TCR(Tmp_file,Fail_file,Trim1, gene,paired,species,primer_file,primer_tag_file,tmp_file,primer_tag_file_count,id,ref_const,reverse_primer_group,cd_hit_directory)
    ...
    Get_read_report(Seq_file1, Seq_file2, Tmp_file, Trim1, nn_orf_filtered,filtering_report,id, species, gene, dir,primer_tag_file_count,primer_file,method,barcode_group,Trim3)
  if(gene.count("TR")!=0 or gene.count("TCR")!=0):
    #Split_reads_and_join(Seq_file1, Seq_file2, Tmp_file,gene,paired,id,method,primer_file,barcode_group,sys.argv[12],dir) 
    #Trim_sequences_BCR_TCR(Tmp_file,Fail_file,Trim1, gene,paired,species,primer_file,primer_tag_file,tmp_file,primer_tag_file_count,id,ref_const,reverse_primer_group,cd_hit_directory)
    Filter_IgJ_genes(Trim1, Trim2, refj,primer_file,ref_const,primer_tag_file_count,blastall,refv,primer_length)
    Reduce_sequences(Trim2, Trim3,primer_file)
