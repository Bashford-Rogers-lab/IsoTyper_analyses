    ...
    if(clas in subsample_depth):
      ...
      if(depth <= len(seq_types[clas])):
        ...
        for r in range(0,repeats):
          ...
          for end in range(depth,len(sample)):
...
def Get_expanded_cluster_summary(cluster_file,sample, cluster_isotype_expansion_SUBSAMPLE_file):
  ...
  out = "#sample\tisotype\texpansion_threshold\tnumber_reads_0.5%\tnumber_reads_5%\n"
  for i in range(0,len(classes)):
    out=out+sample+"\t"+classes[i]+"\t"+str(threshold1)+"\t"+str(freqs1[i])+"\t"+str(freqs2[i])+"\n"
...
def Get_reference_sequence_families(cd_ref_file):
  V_families = {}
  fh=open(cd_ref_file+"LIBRARY/CD_hit_grouping_HOMO_SAPIENS_IGHV.txt","r")
...
def Classify_sequences_into_developmental_stages_per_cluster(sample, annot_file_IMGT, cluster_file,per_cluster_developmental_classification_file,reverse_primer_group):
  ...
  for c in clusters: 
    if(len(clusters[c])>0):
      ...
      chains_used = np.sort(np.unique(chains[nz]))
      classifiations=[chain]
      if(chain in ["IGHD,IGHM","IGHD","IGHM"] and mm <=3):classifiations.append("IGHD,IGHM_unmutated")
      elif(chain in ["IGHD,IGHM","IGHD","IGHM"] and mm >3):classifiations.append("IGHD,IGHM_mutated")
      #if(mm > 0):classifiations.append("mutated")
      MD = 0
      if("IGHD" in chains_used):MD = MD+1
      if("IGHM" in chains_used):MD = MD+1
...
command_source = ["1"]
cd_hit_dir = '/users/immune-rep/mfj169/REPERTOIRE_ANALYSES/BIN/cd-hit-v4.5.7-2011-12-16/'
cd_ref_file = '/users/immune-rep/mfj169/REPERTOIRE_ANALYSES/'
